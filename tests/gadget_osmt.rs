...
impl<'a> OptmzSparseMerkleTree<'a> {
		...
		for i in 0..self.depth {
			...
			if v.0.is_some() {
				...
				if scl == v.1 {
					...
					println!(
						"1Child:found 0 for key val {:?}={:?}",
						scalar_to_u64_array(&v.1),
						scalar_to_u64_array(&v.2)
					);
					cur_node = Scalar::zero();
					break;
				}
	...
	fn _update(&mut self, path: &ScalarBits, val: Scalar, root: &Scalar, depth: usize) -> Scalar {
		println!(
			"Called _update with key-val {:?}={:?}",
			scalar_to_u64_array(&path.to_scalar()),
			scalar_to_u64_array(&val)
		);
		if depth == self.depth {
			return val;
		}
		...
		if *root == self.empty_tree_hashes[depth] {
			println!(
				"Calling get_subtree_with_one_val with depth {} key-val {:?}={:?}",
				depth,
				scalar_to_u64_array(&path.to_scalar()),
				scalar_to_u64_array(&val)
			);
		...
		if child.0.is_some() {
			return self.update_one_val_subtree(
				path,
				val.clone(),
				//&ScalarBits::from_scalar(&child.1), child.2.clone(), depth)
				&ScalarBits::from_scalar_dont_reduce(&child.1),
				child.2.clone(),
				depth,
			);
		} else {
	...
	fn update_one_val_subtree(
		&mut self,
		path_for_new_key: &ScalarBits,
		val_for_new_key: Scalar,
		path_for_old_key: &ScalarBits,
		val_for_old_key: Scalar,
		depth: usize,
	) -> Scalar {
		...
		println!(
			"Called update_one_val_subtree with new key-val {:?}={:?}",
			debug_path_for_new_key,
			scalar_to_u64_array(&val_for_new_key)
		);
		println!(
			"Called update_one_val_subtree with old key-val {:?}={:?}",
			debug_path_for_old_key,
			scalar_to_u64_array(&val_for_old_key)
		);
		if depth == self.depth {
		...
		let (left, right) = {
			...
			if path_for_new_key.is_msb_set() {
				if path_for_old_key.is_msb_set() {
					...
					let left_subtree_hash =
						self.get_subtree_with_one_val(&next_old_path, &val_for_old_key, depth + 1);
					println!(
						"Calling get_subtree_with_one_val with depth {} key-val {:?}={:?}",
						depth + 1,
						debug_next_new_path,
					...
					let right_subtree_hash =
						self.get_subtree_with_one_val(&next_new_path, &val_for_new_key, depth + 1);
					//self.update_db(&left_subtree_hash, (Some(true), next_old_path.to_scalar(), val_for_old_key));
					self.update_db(
						&left_subtree_hash,
						(
							Some(true),
							next_old_path.to_non_reduced_scalar(),
							val_for_old_key,
						),
					);
					...
					self.update_db(
						...
						(
							Some(true),
							next_new_path.to_non_reduced_scalar(),
							val_for_new_key,
						),
					);
	...
	fn update_db(&mut self, key: &Scalar, val: DBVal) {
		...
		println!(
			"Adding to db key-val {:?}=({:?}, {:?}, {:?})",
			scalar_to_u64_array(key),
			&val.0,
			scalar_to_u64_array(&val.1),
			scalar_to_u64_array(&val.2)
		);
...
mod tests {
	...
	fn test_optmz_sparse_merkle_tree() {
		...
		let constants = (0..MIMC_ROUNDS).map(|_| Scalar::one()).collect::<Vec<_>>();
		...
		let kvs: Vec<(Scalar, Scalar)> = (0..10)
		...
		for i in 0..kvs.len() {
			println!(
				"Setting {:?}={:?}",
				scalar_to_u64_array(&kvs[i].0),
				scalar_to_u64_array(&kvs[i].1)
			);

...
mod tests {
	...
	fn set_non_membership_check_gadget() {
		...
		assert!(
			set_non_membership_gadget(&mut prover, var_value, diff_vars, diff_inv_vars, &set)
				.is_ok()
		);
		...
		for i in 1..set_length + 1 {
			let var_diff = verifier.commit(comms[2 * i - 1]);
			diff_vars.push(var_diff);

			let var_diff_inv = verifier.commit(comms[2 * i]);
			diff_inv_vars.push(var_diff_inv);
		}
